...
const products = {
    ...
    'dual-side-binding': {
        ...
        video: 'Video/dual-side-binding.mp4', // Add your video URL here
        images: [
...
function loadProduct(productId) {
    ...
    try {
        ...
        // Update main image with transition styles
        ...
        // Initialize scroll-based gallery
...
// Function to change the main image with animation
function changeImage(element, index) {
    ...
    if (currentSrc !== element.src) {
        ...
        }, 150); // Faster transition for better response
    }
...
// Initialize scroll-based image changing
function initializeScrollGallery() {
    const productGallery = document.querySelector('.product-gallery');
    const thumbnails = document.querySelectorAll('.thumbnail img');
    if (!productGallery || thumbnails.length <= 1) return;
    ...
    productGallery.addEventListener('wheel', (e) => {
        e.preventDefault(); // Prevent page scrolling

        // Accumulate scroll delta
        scrollAccumulator += Math.abs(e.deltaY);

        // If we've accumulated enough scroll distance
        ...
        // Reset accumulator after a period of no scrolling
        scrollTimeout = setTimeout(() => {
            scrollAccumulator = 0;
        }, 50); // Short timeout for fast scrolling response
    }, { passive: false });
}
